...
std::vector<Segment> explore_path(
    ...
    // Extrahiere die Werte aus dem gegebenen Segmentknoten.
    int m = node.first;     // Aktuelle Position im Pfad.
    int index = node.second; // Aktueller Index.

    // Wenn die Position negativ ist, gibt es keinen gültigen Pfad --> Kein Ergebnis
    if (m < 0) {
        return {};
    }

    // Initialisiere einen Vektor
    std::vector<Segment> filtered_array;
    
    // Wenn die Position am Ende des Pfads ist, füge den ersten Sprung in die Liste der möglichen Schritte ein
    if (m == n - 1) {
        filtered_array.push_back(jumps_made.at(m)[0]);
    } else {
        // Durchlaufe die Sprünge dieses Schrittes
        // falls Werte nicht in der Liste ---> hinzufügen in banned list
    ...
    // Durchlaufe die Liste der möglichen nächsten Schritte
    for (size_t i = 0; i < filtered_array.size(); ++i) {
        const Segment& segment = filtered_array[i];
        
        // Erstelle den nächsten Knoten basierend auf dem aktuellen Index und der Position im Pfad
        Segment next_node = make_segment(m - 1, i);
        
        // Aktualisiere die Liste der verbotenen Elemente
        std::vector<int> next_banned = banned;
        next_banned.push_back(segment.first);
        next_banned.push_back(segment.second);
        
        // Aktualisiere den Pfad mit dem aktuellen Schritt
        std::vector<Segment> new_path = path;
        new_path.push_back(segment);

        // Wenn der Pfad die gewünschte Länge erreicht hat, gib den Pfad als Lösung zurück.
        if (new_path.size() == static_cast<size_t>(n)) {
            return new_path;
        }

        // Rekursiver Aufruf wie in Informatik
...
void log_solutions(const std::vector<Segment>& solutions, int n) {
    std::ofstream logfile("solutions_log.txt", std::ios_base::app);
    if (logfile.is_open()) {
        logfile << "Lösungen for n = " << n << ":\n";
        logfile << "Array representation: [";
        for (size_t i = 0; i < solutions.size(); ++i) {
            logfile << "(" << solutions[i].first << ", " << solutions[i].second << ")";
...
int main() {
    ...
    for (int n = start_n; n <= end_n; ++n) {
        ...
        if (gsl.empty()) {
            ...
            std::cout << "Eine Lösung:\n";
            std::cout << "Array representation: [";
            for (size_t i = 0; i < gsl.size(); ++i) {
                std::cout << "(" << gsl[i].first << ", " << gsl[i].second << ")";
                if (i < gsl.size() - 1) {
                    std::cout << ", ";
                }
            }
